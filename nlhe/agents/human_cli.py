...
def _fmt_options(engine: EngineLike, state: GameState, seat: int):
...
class HumanAgent(Agent):
    def act(self, engine: EngineLike, state: GameState, seat: int) -> Action:
        assert state.next_to_act == seat
        options, info = _fmt_options(engine, state, seat)
        ...
        while True:
            ...
            if any(a.kind==ActionType.RAISE_TO for a in acts):
                ...
                if raw.startswith("r ") or raw.startswith("raise ") or raw.startswith("bet "):
                    ...
                    if amt <= state.current_bet: print(f"Amount must exceed current bet ({state.current_bet})."); continue
