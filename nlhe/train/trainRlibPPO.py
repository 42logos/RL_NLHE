...
if __name__ == "__main__":
    ...
    config = (
        ...
        .training(
            # —— 学习率与步幅（先稳住）
            lr=1e-4,                  # 先把 3e-4 降一档
            num_epochs=3,             # 10 → 3，防止过拟合/overshoot
            train_batch_size_per_learner=16384,  # 1024 → 16384（或你显存允许的较大值）
            minibatch_size=2048,      # 512 → 2048

            # —— PPO 稳定性
            grad_clip=0.3,            # 0.5 → 0.3
    ...
    for i in range(1435):
        ...
        if i % 10 == 0:
            ...
            # 用 URI 最稳
            cp = algo.save_to_path(savepoint)
